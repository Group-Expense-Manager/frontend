...
export default function useChangePassword(oldPassword: string, newPassword: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      router.push('/you/password-change/password-changed-successfully-modal');
    },

    onError: (error: AxiosError) => {
      if (error.response?.status === 400) {
        router.push('/you/password-change/wrong-old-password-modal');
      } else {
        router.push('/you/password-change/error-modal');
      }
    },
  });
}
