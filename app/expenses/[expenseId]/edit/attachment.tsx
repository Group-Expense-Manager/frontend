...
export default function EditExpenseAttachment() {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      await handleImageChoice(
        result.assets[0],
        () => router.push(`/expenses/${params.expenseId}/edit/unsupported-file-format-modal`),
      ...
      <View className="py-8 w-full h-full flex flex-col justify-between items-center">
        <View className="w-full space-y-3 flex-1">
          ...
          <View className="flex-1 justify-center">
            <AttachmentButton
              ...
              image={expenseUpdate.newAttachment}
            />
          </View>
        </View>
        ...
        <View className="py-8 w-full flex flex-col justify-center items-center space-y-8">
          <View className="w-full">
            <CustomButton
              onPress={() => router.push(`/expenses/${params.expenseId}/edit/message`)}
              title={t('Next')}
            />
          </View>
          <View className="w-full">
